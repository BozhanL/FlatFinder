...
// Define a common interface for errors that carry a code (like Firebase/Google errors)
interface CodeError {
    code: string;
    message?: string;
}

// 1. Interface for props used by UserStatusScreen
...
const UserStatusScreen = ({ user, handleSignOut }: UserStatusScreenProps): JSX.Element => (
...
const AuthScreen = (): JSX.Element => {
    ...
    useEffect(() => {
        ...
        const subscriber = onAuthStateChanged(authInstance, (currentUser) => {
            // TS Fix: currentUser is correctly typed (User | null)
            setUser(currentUser);
            if (isAuthLoading) {
    ...
    const handleAuth = async () => {
        ...
        try {
            ...
            // TS Fix: Use type guard for safe property access
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            // 3. Robust token retrieval (TS Fix: Use 'as any' to bypass type definition error)
            ...
            // TS Fix: Use type guard to safely check error codes from Google/Firebase
            if (isGoogleError(e)) {
