...
const NZ_REGIONS: { region: string; items: string[] }[] = [
  { region: "Auckland", items: ["Auckland CBD", "North Shore", "West Auckland", "South Auckland", "East Auckland"] },
  { region: "Waikato", items: ["Hamilton", "Cambridge", "Taupō", "Te Awamutu"] },
  { region: "Bay of Plenty", items: ["Tauranga", "Mount Maunganui", "Whakatāne", "Rotorua"] },
  { region: "Wellington", items: ["Wellington CBD", "Lower Hutt", "Upper Hutt", "Porirua", "Kapiti"] },
  { region: "Canterbury", items: ["Christchurch CBD", "Riccarton", "Addington", "Rolleston"] },
...
const FLATTENED = NZ_REGIONS.flatMap(g => [
  ...
  ...g.items.map(name => ({ type: "item" as const, key: `${g.region}-${name}`, label: name }))
]);
...
export default function NZLocationPickerField({
  ...
  const data = useMemo(() => {
    ...
    const matched = NZ_REGIONS.flatMap(g => {
      ...
      const items = g.items.filter(x => inGroup || x.toLowerCase().includes(kw));
      if (!inGroup && items.length === 0) return [];
      return [
        { type: "header" as const, key: `h-${g.region}`, label: g.region },
        ...items.map(name => ({ type: "item" as const, key: `${g.region}-${name}`, label: name }))
      ];
    });
    return matched.length ? matched : [{ type: "header" as const, key: "none", label: "No results" }];
  }, [q]);

  return (
    <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
      <Text style={{ fontSize: 14, fontWeight: "700", marginBottom: 8 }}>{label}</Text>
      ...
      <TouchableOpacity
        ...
        style={styles.input}
      >
        <Text style={{ color: value ? "#111" : "#999" }}>{value || "Select a region / area"}</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  sheet: { backgroundColor: "#fff", borderTopLeftRadius: 14, borderTopRightRadius: 14, maxHeight: "75%" },
  sheetHeader: {
    height: 48, paddingHorizontal: 12, alignItems: "center",
  ...
  header: { backgroundColor: "#F6F6F6", paddingVertical: 6, paddingHorizontal: 12 },
